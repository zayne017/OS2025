...
sfs_bmap_get_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, uint32_t index, bool create, uint32_t *ino_store) {
    ...
    if (index < SFS_NDIRECT) {// 如果你要找的页号在 0~11 之间 (SFS_NDIRECT = 12)
      // 1. 查表：直接看 din->direct 数组里有没有记录
        if ((ino = din->direct[index]) == 0 && create) {
            // 2. 如果是 0 (没分配) 且你需要创建 (create=true)
            // 3. 分配：去问位图要一个新的磁盘块，放到 ino 里
            if ((ret = sfs_block_alloc(sfs, &ino)) != 0) {
                return ret;// 硬盘满了，分配失败
            }
            // 4. 记录：把新申请到的硬盘号，填回数组
            din->direct[index] = ino;
            sin->dirty = 1;// 标记：我改了 Inode，记得存盘
        }
        goto out;
    }
    // the index of disk block is in the indirect blocks.
    // 如果 index >= 12，说明是后面的大块数据
    index -= SFS_NDIRECT;// 比如你要找第 13 块，减去 12，等于间接块里的第 1 块。
    if (index < SFS_BLK_NENTRY) {
        ent = din->indirect;// 1. 拿钥匙：ent 是“间接索引块”的地址
        // 2. 找下级：调用子函数去那个索引块里找
    // 注意：这里把 &ent 传进去了。如果 ent 是 0（之前没有间接块），
    // 子函数会分配一个新的索引块，并把新块号填给 ent。
        if ((ret = sfs_bmap_get_sub_nolock(sfs, &ent, index, create, &ino)) != 0) {
            return ret;
        }
        // 3. 挂载：如果 ent 变了（说明原本没有间接块，是刚才新分配的）
        if (ent != din->indirect) {
            assert(din->indirect == 0);
            din->indirect = ent;// 把这个新索引块挂到 Inode 上
            sin->dirty = 1;
        }
        goto out;
    } else {
...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    ...
    // 计算结束位置 endpos = 开始位置 offset + 要读写的长度 *alenp
    off_t endpos = offset + *alenp, blkoff;
    *alenp = 0;// 先把实际完成长度置 0，后面累加
	// calculate the Rd/Wr end position
    // --- 边界检查 ---
    // 1. offset < 0: 偏移量不能是负数
    // 2. offset >= SFS_MAX_FILE_SIZE: 不能超过文件系统支持的最大文件大小
    ...
    // --- 读操作的特殊检查 ---
    if (!write) {
        // 如果读的起始位置已经超过文件大小，说明没东西可读了
        if (offset >= din->size) {
            return 0;
        }
        // 如果想读的结束位置超过了文件大小，那只能读到文件末尾为止
        if (endpos > din->size) {
            endpos = din->size;
        }
    }
    // 定义两个函数指针：
    // buf_op: 用于读写非对齐的碎片数据 (buffer operation)
    // block_op: 用于读写整块数据 (block operation)
    int (*sfs_buf_op)(struct sfs_fs *sfs, void *buf, size_t len, uint32_t blkno, off_t offset);
    int (*sfs_block_op)(struct sfs_fs *sfs, void *buf, uint32_t blkno, uint32_t nblks);
    if (write) {
        sfs_buf_op = sfs_wbuf, sfs_block_op = sfs_wblock;// 写模式，挂载写函数
    }
    else {
        sfs_buf_op = sfs_rbuf, sfs_block_op = sfs_rblock;// 读模式，挂载读函数
    }
    ...
    uint32_t ino;
    // blkno: 起始的逻辑块号。
    // 例如：offset = 4098，则 blkno = 1 (因为 4098 / 4096 = 1)
