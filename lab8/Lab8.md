...
struct pipe_inode_info {
    ...
    unsigned int waitwriter  //等待写的数量s  
...
在 ucore 中实现了一个简化的文件系统，并将其集成到虚拟文件系统 VFS 框架中，使操作系统具备了从磁盘加载 ELF 文件并执行的能力，最终实现了一个支持基本交互操作的 Shell。

lab8 体现了一切皆文件的思想，通过 VFS 的抽象，普通文件、目录、甚至硬件设备如 `stdin/stdout`都被统一视为 `inode`。

总结操作系统处理文件的四层结构：

用户接口层：

- 提供 `open`, `read`, `write`, `close` 等系统调用。
- 引入了文件描述符 fd 的概念，是进程打开文件表 (`fd_array`) 的数组下标，实现了用户态与内核态对象的隔离与安全访问。

虚拟文件系统层 VFS：

- 屏蔽了底层具体文件系统的差异。
- 维护 `struct file`（记录读写位置 `pos`、访问权限）和 `struct inode`（内存中的文件节点）。
- 负责管理文件描述符表，完成从 `fd` 到 `inode` 的映射。

SFS 文件系统层 SFS：

- 通过 `sfs_disk_inode` 记录文件信息，通过 `sfs_disk_entry` 实现目录与文件的映射。

设备 I/O 层：

- 通过接口对接底层块设备驱动，完成对物理扇区的读写。



至此，我们本学期的 OS 实验到此结束，从最小可执行内核开始，到内存管理，处理中断，调试 GDB，实现进程管理和完成不同的调度算法，直到文件系统管理，最终实现了一个可以运行简单命令行的操作系统。在实验中，我们将理论转化为具体的代码实现，深刻理解操作系统并发、共享、虚拟、异步的基本特征，理解软硬件接口的设计原则以及各子系统协同工作的底层机制，提升了设计与调试复杂系统的能力。
