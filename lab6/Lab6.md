...
我们在 ucore 中额外实现了两种基础调度算法：

1.  **FIFO (First-In-First-Out)**:
    *   **策略**: 按照进程进入就绪队列的顺序进行调度，先到先服务。
    *   **实现**: 使用链表维护就绪队列，`enqueue` 插入队尾，`pick_next` 选择队头。非抢占式。
    *   **代码**: `kern/schedule/sched_fifo.c`

2.  **SJF (Shortest Job First)**:
    *   **策略**: 选择预计运行时间（Burst Time）最短的进程优先执行。
    *   **实现**: 使用有序链表维护就绪队列。在 `enqueue` 时，根据进程的 `lab6_priority` (模拟 Burst Time) 将其插入到链表的正确位置，保持链表按 Burst Time 升序排列。`pick_next` 选择队头。非抢占式。
    *   **代码**: `kern/schedule/sched_sjf.c`

### 2. 测试用例设计

为了验证算法并进行定量分析，我们设计了三个测试用例（位于 `user/` 目录下）：

1.  **`test_fifo.c`**:
    *   **场景**: 模拟护航效应 (Convoy Effect)。
    *   **逻辑**: 创建 3 个子进程。Child 0 为长作业（模拟高负载），Child 1 和 Child 2 为短作业。它们按顺序创建并进入就绪队列。
    *   **预期**: Child 0 先到达并长时间占用 CPU，导致后续短作业 Child 1 和 Child 2 被迫等待，周转时间显著增加。

2.  **`test_sjf.c`**:
    *   **场景**: 验证短作业优先特性。
    *   **逻辑**: 创建 3 个子进程，分别设置优先级 100 (模拟长作业), 10 (模拟短作业), 15 (模拟短作业)。所有进程执行相同的计算量。
    *   **预期**: 调度器应优先选择优先级数值小（代表作业短）的进程。执行顺序应为 Child 1 (10) -> Child 2 (15) -> Child 0 (100)。

3.  **`test_stride.c`**:
    *   **场景**: 验证比例共享特性。
    *   **逻辑**: 创建 3 个子进程，优先级分别为 2, 3, 5。进程并发运行固定时间。
    *   **预期**: 获得的 CPU 时间（计数器值）比例应接近 2:3:5，验证 Stride 算法的比例共享特性。
...
**1. Stride 算法 (`test_stride`)**
*   **设置**: 3 个子进程，优先级分别为 2, 3, 5。
*   **结果**:
    *   Child 0 (Prio 2): Count 284,029 (Scaled 28)
    *   Child 1 (Prio 3): Count 394,524 (Scaled 39)
    *   Child 2 (Prio 5): Count 588,497 (Scaled 58)
*   **比例分析**:
    *   理论比例: $2 : 3 : 5 = 1 : 1.5 : 2.5$
    *   实际比例: $28 : 39 : 58 \approx 1 : 1.39 : 2.07$
    *   **结论**: 实际运行结果与理论优先级高度正相关，验证了 Stride 算法实现了按权重的比例共享调度。

**2. FIFO 算法 (`test_fifo`)**
*   **设置**: 
    *   Child 0: 长作业 (模拟高负载)。
    *   Child 1, Child 2: 短作业 (模拟低负载)。
    *   到达顺序: Child 0 -> Child 1 -> Child 2。
*   **结果**:
    *   **开始顺序**: Child 0 (10ms) -> Child 1 (20ms) -> Child 2 (40ms)。
    *   **完成顺序**: Child 0 (700ms) -> Child 2 (890ms) -> Child 1 (1080ms)。
    *   **周转时间**: Child 0 (700ms), Child 2 (~880ms), Child 1 (~1070ms)。
*   **现象分析**:
    *   **护航效应 (Convoy Effect)**: 最新的测试结果完美复现了 FIFO 算法的这一典型特征。
    *   **长作业阻塞**: Child 0 作为长作业最先获得 CPU，它一直运行直到 700ms 结束。在此期间，Child 1 和 Child 2 尽管已经创建并处于就绪状态，但无法获得 CPU 资源。
    *   **短作业受损**: Child 1 和 Child 2 本身仅需约 100ms 的运行时间。然而，由于必须等待 Child 0 完成，它们的实际完成时间被推迟到了 890ms 和 1080ms。
    *   **结论**: 这一结果证实了 FIFO 算法严格遵循“先来先服务”原则。虽然保证了公平性（按顺序），但严重损害了短作业的响应时间和系统的平均周转时间。

**3. SJF 算法 (`test_sjf`)**
*   **设置**: 3 个子进程，优先级分别为 2 (高/短作业), 3, 5 (低/长作业)。
*   **结果**:
    *   Child 0 (Prio 2): Count **1,274,243** (Scaled 127)
    *   Child 1 (Prio 3): Count **14,336** (Scaled 1)
    *   Child 2 (Prio 5): Count **18,771** (Scaled 1)
*   **现象分析**:
    *   高优先级（模拟短作业）的 Child 0 获得了绝大多数 CPU 时间（是其他进程的 ~70 倍）。
    *   低优先级的 Child 1 和 Child 2 几乎处于**饥饿 (Starvation)** 状态。
    *   **结论**: SJF 算法极度偏向于短作业（或高优先级作业）。虽然这能最小化平均周转时间（让短作业尽快完成），但会导致长作业无法获得服务。

#### 3.2 综合对比与适用范围

| 算法 | 核心策略 | 优点 | 缺点 | 适用场景 |
| :--- | :--- | :--- | :--- | :--- |
| **FIFO** | 先来先服务 | 实现简单，低开销 | 存在**护航效应** (Convoy Effect)，短作业可能被长作业阻塞；响应时间差 | 批处理系统，后台任务，负载较轻且任务长度差异不大的场景 |
| **SJF** | 短作业优先 | **平均周转时间最优**；系统吞吐量高 | 需要预知作业长度；长作业可能**饥饿**；实现复杂 | 作业长度可预测的批处理系统 |
| **Stride** | 比例共享 | **公平性好**；可精确控制资源分配；响应时间较好 | 实现相对复杂（需要维护斜堆等结构）；有一定额外开销 | 通用操作系统，多媒体应用，需要服务质量保证 (QoS) 的场景 |

### 4. 总结

通过本次实验，我们在 ucore 上成功实现了 FIFO 和 SJF 调度算法，并与原有的 Stride 算法进行了对比。
*   **Stride** 展现了优秀的比例共享能力，适合现代多任务环境。
*   **SJF** 展示了对高优先级任务的绝对偏好，验证了其在优化周转时间方面的潜力，但也暴露了严重的公平性问题。
*   **FIFO** 在本实验中表现出了类似 RR 的特性（由于抢占机制），说明了调度策略与抢占机制配合的重要性。
