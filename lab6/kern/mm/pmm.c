               ...
               bool share)
{
    ...
    do
    {
        ...
        if (*ptep & PTE_V)
        {
            ...
            // 1. 获取源页的内核虚拟地址
            // page 只是个管理结构体，不能直接读写。
            // page2kva (Page to Kernel Virtual Address) 计算出这个物理页在内核里的虚拟地址。
            // 比如：父进程的数据在物理地址 0x1234，内核通过 0xC0001234 这个地址去访问它。
            void *src_kvaddr = page2kva(page);
            // 2. 获取目标页的内核虚拟地址
            // 同样，我们需要拿到新分配那页纸在内核里的写入地址。
            void *dst_kvaddr = page2kva(npage);
            // 3. 内存拷贝
            // 调用 memcpy，把父进程那一页的 4096 字节 (PGSIZE) 数据，
            // 原封不动地拷贝到子进程的新页里。
            memcpy(dst_kvaddr, src_kvaddr, PGSIZE);
            // 4. 建立映射关系
            // 现在的状态是：子进程有了新页 npage，里面装好了数据，但子进程的页表还不知道这件事。
            // page_insert 的作用是：在子进程的页表 (to) 中，把虚拟地址 start 映射到物理页 npage。
            // 同时赋予它权限 perm。
            ret = page_insert(to, npage, start, perm);
            //start虚拟地址 npage物理地址 perm权限
            assert(ret == 0);// 确保映射成功
        }
        start += PGSIZE;
    } while (start != 0 && start < end);
