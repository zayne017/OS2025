...
bool user_mem_check(struct mm_struct *mm, uintptr_t addr, size_t len, bool write)
{
    if (mm != NULL)
    {
        ...
        // 2. 遍历检查每一段内存
        // 因为用户给的范围 [start, end) 可能会跨越多个 VMA（虚拟内存区域），或者中间有空洞。
        while (start < end)
        {
            // find_vma 查找包含 start 的 VMA，或者 start 之后的第一个 VMA。
            // 如果没找到，或者找到的 VMA 起始地址比我们要找的 start 还大（说明中间有未映射的空洞），拒绝。
            if ((vma = find_vma(mm, start)) == NULL || start < vma->vm_start)
            {
                return 0;
            }
            // 3. 权限位检查
            // 如果我们要写 (write=1)，但 VMA 没标记 VM_WRITE，拒绝。
            // 如果我们要读 (write=0)，但 VMA 没标记 VM_READ，拒绝。
