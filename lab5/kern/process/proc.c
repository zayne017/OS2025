...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    //    根据 clone_flags 决定是复制一份新的内存(dup)，还是共享老爸的内存(share)
    ...
    local_intr_save(intr_flag);  // 必须关中断！因为要操作全局链表，防止被打断
    {
        ...
        // 【关键】调用 set_links 设置进程关系链接
        // 这个函数帮我们干了三件事：
        // 1. list_add(&proc_list, ...) 把孩子加入全局进程链表
        // 2. 维护复杂的父子兄弟链表 (parent, cptr, optr, yptr)
        // 3. nr_process++ (进程总数加 1)
        set_links(proc);
    }
...
int do_exit(int error_code)
{
    ...
    // 把用户传进来的 error_code (比如 exit(0) 里的 0) 存起来。
...
load_icode(unsigned char *binary, size_t size)
{
    ...
    int ret = -E_NO_MEM;
    struct mm_struct *mm;
    //(1) create a new mm for current process 创建新的 mm (内存管理器)
    if ((mm = mm_create()) == NULL)
    {
        goto bad_mm;
    }
    //(2) create a new PDT, and mm->pgdir= kernel virtual addr of PDT 创建新的页表
    if (setup_pgdir(mm) != 0)
    {
        goto bad_pgdir_cleanup_mm;
    }
    ...
    //(3.3) This program is valid? 确认这真的是个 ELF 文件，不是乱码
    ...
    for (; ph < ph_end; ph++)// 遍历每一个段
    {
        ...
        //(3.6.1) copy TEXT/DATA section of bianry program 把二进制文件里的代码搬进内存
        while (start < end)
        {
        ...
        // 更新 end：现在 end 变成了整个段（包含BSS）的结束地址
        // p_memsz 通常大于 p_filesz，差值就是 BSS
        end = ph->p_va + ph->p_memsz;
        if (start < la)
        {
            // 如果 memsz == filesz，说明没 BSS，直接跳过
    ...
    //(4) build user stack memory 第四步：铺设用户栈
    ...
    lsatp(PADDR(mm->pgdir));// 切换CPU新页表 告诉 MMU（内存管理单元），以后请用这张新表来翻译地址

    //(6) setup trapframe for user environment
    struct trapframe *tf = current->tf;
    //tf 指针指向的是旧进程（user_main）遗留下来的中断帧
    // Keep sstatus
     ...
     */
    // 设置用户栈指针
    // 刚才我们建立了用户栈，范围是 [USTACKTOP-USTACKSIZE, USTACKTOP)
    // 栈是向下增长的，所以栈顶指针必须指向最高地址 USTACKTOP
    tf->gpr.sp = USTACKTOP;
    // 设置入口地址
    // 当 sret 返回用户态时，CPU 会跳转到 sepc 指向的地址开始执行。
    // ELF 文件的头里 e_entry 字段记录了程序的入口地址
    tf->epc = elf->e_entry;
    // 设置 sstatus 寄存器
    // 需要构造一个合适的 sstatus 值，告诉 CPU：
    // (1) SSTATUS_SPP = 0:  意味着“刚才”是用户态。这样 sret 就会返回到用户态。
    // (2) SSTATUS_SPIE = 1: 意味着“刚才”中断是开着的。这样返回用户态后，中断会被自动打开，
    //                       否则进程一跑起来就屏蔽了中断，操作系统就没法抢占调度了。
    // read_csr(sstatus): 读取当前寄存器的值作为底板
